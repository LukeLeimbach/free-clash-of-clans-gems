...
const applyParticleEffect = (
  ...
  function generateParticle() {
    ...
    if (particleType === "circle") {
      ...
      // Add scale animation for image particles
      const scaleAnimationDuration = 300 // milliseconds
      const startTime = performance.now()
      
      const particleData: CoolParticle = {
        direction,
        element: particle,
        left,
        size,
      ...
      const animateScale = () => {
        ...
        const progress = Math.min(elapsed / scaleAnimationDuration, 1)
        
        let currentScale = 1
        if (progress < 0.5) {
          // Scale up from 1 to 1.1 (first half)
          currentScale = 1 + (0.1 * (progress * 2))
        } else {
          // Scale down from 1.1 to 1 (second half)
          currentScale = 1.1 - (0.1 * ((progress - 0.5) * 2))
        }
        
        // Find the particle by reference
        const p = particles.find(part => part === particleData)
        if (p && p.scaleAnimationStart === startTime) {
          p.scale = currentScale
          
          if (progress < 1) {
            requestAnimationFrame(animateScale)
          } else {
            // Animation complete, remove scale animation properties
            p.scale = 1
            p.scaleAnimationStart = undefined
          }
        }
      }
    ...
    particles.forEach((p) => {
      ...
      p.element.setAttribute(
        ...
        [
          ...
          `top:${p.top}px`,
          `left:${p.left}px`,
          `transform:rotate(${p.spinVal}deg) scale(${scale})`,
        ].join(";")
      )
    })
  }
